...
A ChoiceField featuring a user-submitted "other" value.

Adapted from https://www.djangosnippets.org/snippets/863/
...
class ChoiceWithOtherRenderer(forms.RadioSelect.renderer):
    ...
    def __iter__(self):
        for input in super(ChoiceWithOtherRenderer, self).__iter__():
            yield input
        _id = '%s_%s' % (self.attrs['id'], self.other[0]) if 'id' in self.attrs else ''
        label_for = ' for="%s"' % _id if _id else ''
        checked = (
            ''
            if not force_text(self.other[0]) == self.value
            else 'checked="true" '
        )
        yield mark_safe(
            f'<label{label_for}><input type="radio" id="{_id}"'
            f' value="{self.other[0]}" name="{self.name}" {checked}/>'
            f' {self.other[1]} </label> %%s')
...
class ChoiceWithOtherWidget(forms.MultiWidget):
    ...
    def __init__(self, choices):
        widgets = [
            forms.RadioSelect(
                choices=choices, renderer=ChoiceWithOtherRenderer),
            forms.TextInput,
        ]
...
specify:</label> <input type="text" name="age_1" id="id_age_1" /></li>
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(OtherChoiceField, self).__init__(widget=widget, fields=fields, *args, **kwargs)
